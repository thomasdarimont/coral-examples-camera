...
def draw_pose(cv2TargetImage, pose, color='yellow', threshold=0.2):
    xys = {}
    for label, keypoint in pose.keypoints.items():
        if keypoint.score < threshold: continue
        xys[label] = (int(keypoint.yx[1]), int(keypoint.yx[0]))

        cv2.circle(cv2TargetImage, (int(keypoint.yx[1]), int(keypoint.yx[0])), radius=5, color=(0,255,255), thickness=-1)
    ...
    if not (xysNose is None or xysLeftEar is None or xysRightEar is None):
        ...
        cv2.circle(cv2TargetImage, (xysNose[0], xysNose[1]), radius=int(dxEars/1.4), color=(0,255,255), thickness=-1)
